...
VENDOR_ID = os.getenv("VENDOR_ID", "VN72123277")
CURRENCY = os.getenv("CURRENCY", "UAH")
PDV_RATE = int(os.getenv("PDV_RATE", "20"))
...
def process_file(file_path):
    ...
    filename = f"{VENDOR_ID}_{today}_{now_time}.csv"
...
def send_email(file_path):
    if not EMAIL_SENDER:
        raise ValueError("❌ EMAIL_SENDER не найден")

    if not EMAIL_PASSWORD:
        raise ValueError("❌ EMAIL_APP_PASSWORD не найден")

    if not EMAIL_RECEIVER:
        raise ValueError("❌ EMAIL_RECEIVER не найден")
